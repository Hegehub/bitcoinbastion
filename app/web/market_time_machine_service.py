...
class MarketTimeMachineWebService:
    ...
    def dashboard(self, *, timeframe: str = "1h", page_size: int = 80) -> MarketTimelineDTO:
        ...
        return MarketTimelineDTO(
            ...
            candles=candles,
            signals=[
                SignalCardDTO(title="No candidate signals currently available.", status="empty")
            ],
            page=1,
            page_size=50,
    ...
    def landing_payload(self, *, timeframe: str = "1h") -> dict[str, object]:
        ...
        return {
            ...
            "btc_price": {
                ...
                "provider_confidence": (
                    latest_candle.get("provider_confidence") if latest_candle else 0.0
                ),
            },
    ...
    def signal_summary(self, *, limit: int = 50, status: str | None = None) -> dict[str, object]:
        ...
        rows = list(
            self.db.execute(
                query.order_by(
                    IntelligenceSignalCandidate.created_at.desc(),
                    IntelligenceSignalCandidate.id.desc(),
        ...
        pending_ids = [
            row.id
            for row in rows
            if row.requires_operator_review or self._signal_bucket(row) == "pending_review"
        ]
        reviews = list(
            self.db.execute(
        ...
        return {
            "items": items,
            "counts": counts,
            "latest_published_signal": next(
                (item for item in items if item["status"] == "published"),
    ...
    def evidence_summary(self, *, limit: int = 25) -> dict[str, object]:
        ...
        artifacts = list(
            self.db.execute(
                ...
                .where(EvidenceArtifact.packet_id.in_(packet_ids or [0]))
                .limit(200)
            ).scalars()
        )
        ...
        return {
            ...
            "limitations": SAFETY_LIMITATIONS
            + ([] if packets else ["No evidence packets available."]),
        }
    ...
    def replay_requests_summary(self, *, limit: int = 25) -> dict[str, object]:
        rows = list(
            self.db.execute(
                ...
                .order_by(EvidenceReplayLog.started_at.desc(), EvidenceReplayLog.id.desc())
                .limit(min(max(limit, 1), 100))
            ).scalars()
        )
        return {
            "items": [
                {
                    ...
                    "publication_status": (row.metadata_json or {}).get(
    ...
    def source_summary(self, *, limit: int = 50, sort: str = "name") -> dict[str, object]:
        rows = list(
            self.db.execute(
                select(NewsSource).order_by(NewsSource.name.asc()).limit(min(max(limit, 1), 100))
            ).scalars()
        )
        ...
        reputations = {
            ...
            for row in self.db.execute(
                select(SourceReputationProfile).where(
                    SourceReputationProfile.source_id.in_(source_ids or [0])
                )
            ).scalars()
        }
            ...
            items.append(
                {
                    ...
                    "signal_quality": (
                        reputation.signal_quality_score if reputation else row.signal_quality_weight
                    ),
        ...
        if sort == "confidence":
            items.sort(
                key=lambda item: self._float_value(item.get("provider_confidence")), reverse=True
            )
        elif sort == "failures":
        ...
        return {
            ...
            "provider_health": {
                ...
                "provider_confidence": (
                    round(
                        sum(self._float_value(item.get("provider_confidence")) for item in items)
                        / len(items),
                        4,
                    )
                    if items
                ...
                "degraded_sources": [
                    str(item.get("source_name")) for item in items if bool(item.get("is_degraded"))
                ],
            ...
            "limitations": SAFETY_LIMITATIONS
            + ([] if items else ["Source registry is empty or unavailable."]),
        }
    ...
    def timeline(
        ...
        *,
        ...
        query = select(IntelligenceTimelineEvent).where(
        ...
        order = (
            IntelligenceTimelineEvent.event_time.asc()
            if sort == "asc"
            else IntelligenceTimelineEvent.event_time.desc()
        )
        rows = list(
            self.db.execute(
                query.order_by(order, IntelligenceTimelineEvent.id.desc())
    ...
    def candle_attribution(self, candle_id: int) -> CandleAttributionDTO:
        ...
        events = {
            row.id: row
            for row in self.db.execute(
                select(NewsEvent).where(NewsEvent.id.in_(event_ids or [0]))
            ).scalars()
        }
        articles = {
            row.id: row
            for row in self.db.execute(
                select(NewsArticle).where(NewsArticle.id.in_(article_ids or [0]))
            ).scalars()
        }
        return CandleAttributionDTO(
            id=candle.id,
            ...
            historical_similarity_count=sum(
                1 for c in candidates if (c.metadata_json or {}).get("historical_similarity_count")
            ),
            ...
            candidate_events=[
                self._candidate_event(c, events.get(c.event_id or 0))
                for c in candidates
                if c.event_id
            ],
            candidate_articles=[
                self._candidate_article(c, articles.get(c.article_id or 0))
                for c in candidates
                if c.article_id
            ],
            candidate_news_events=[
                self._candidate_event(c, events.get(c.event_id or 0))
                for c in candidates
                if c.event_id
            ],
            candidate_macro_events=[
                self._candidate_event(c, events.get(c.event_id or 0))
                for c in candidates
                if c.event_id and self._event_is(events.get(c.event_id or 0), "macro")
            ],
            candidate_security_events=[
                self._candidate_event(c, events.get(c.event_id or 0))
                for c in candidates
                if c.event_id and self._event_is(events.get(c.event_id or 0), "security")
            ],
            candidate_narrative_events=[
                self._candidate_event(c, events.get(c.event_id or 0))
                for c in candidates
                if c.event_id and not self._event_is(events.get(c.event_id or 0), "security")
            ],
            top_attribution=(
                self._candidate_event(candidates[0], events.get(candidates[0].event_id or 0))
                if candidates and candidates[0].event_id
                else {}
            ),
            confidence_breakdown={
                "attribution_confidence": max([c.normalized_score for c in candidates] or [0.0]),
                "provider_confidence": candle.provider_confidence,
                "volatility_score": candle.volatility_score,
            },
            ...
            safety_flags={
                ...
                "confidence_score": max([c.normalized_score for c in candidates] or [0.0]),
            },
        )
        ...
        for row in rows:
            ...
            markers.append(
                NewsMarkerDTO(
                    ...
                    provider_confidence=(
                        impact.provider_confidence if impact else row.provider_confidence
                    ),
                    impact_confidence=(
                        impact.impact_confidence_score if impact else row.market_impact_score
                    ),
    ...
    def event_context(self, event_id: int) -> dict[str, object]:
        ...
        marker = next(
            (item for item in self.news_markers(limit=1000) if item.event_id == event_id), None
        )
        return {
            "data": (
                marker.model_dump()
    ...
    def evidence_panel(self, packet_id: int) -> EvidencePanelDTO:
        ...
        return EvidencePanelDTO(
            ...
            evidence_sources=[
                {"type": a.artifact_type, "entity_type": a.entity_type, "entity_id": a.entity_id}
                for a in artifacts
            ],
            provider_confidence=packet.provider_confidence or 0.0,
            source_confidence=packet.source_confidence or 0.0,
            ...
            confidence_breakdown={
                "packet_confidence": packet.confidence_score or 0.0,
                "provider_confidence": packet.provider_confidence or 0.0,
                "source_confidence": packet.source_confidence or 0.0,
            },
            provider_snapshot={
    ...
    def replay_summary(self, entity_type: str, entity_id: int) -> ReplaySummaryDTO:
        rows = list(
            self.db.execute(
                select(EvidenceReplayLog)
                .where(
                    EvidenceReplayLog.entity_type == entity_type,
                    EvidenceReplayLog.entity_id == entity_id,
                )
        ...
        return ReplaySummaryDTO(
            ...
            steps=[
                {
                    "step": r.step_name,
                    "status": "success" if r.success else (r.error_code or "pending"),
                    "created_at": self._dt(r.started_at),
                }
                for r in rows
            ],
            limitations=SAFETY_LIMITATIONS + ([] if rows else ["Replay unavailable."]),
    ...
    def evidence_for_candle(self, candle_id: int) -> EvidencePanelDTO:
        packet = self.db.execute(
            select(EvidencePacket)
            .where(
                EvidencePacket.source_entity_type == "btc_candle",
                EvidencePacket.source_entity_id == candle_id,
            )
            .order_by(EvidencePacket.created_at.desc(), EvidencePacket.id.desc())
            .limit(1)
        ).scalar_one_or_none()
    ...
    def candle_similarity_preview(self, candle_id: int, limit: int = 5) -> list[dict[str, object]]:
        candidate = self.db.execute(
            select(CandleAttributionCandidate)
            .where(
                CandleAttributionCandidate.candle_id == candle_id,
                CandleAttributionCandidate.event_id.is_not(None),
            )
            .order_by(
                CandleAttributionCandidate.normalized_score.desc(),
                CandleAttributionCandidate.id.asc(),
    ...
    def narrative_panel(self, *, limit: int = 9) -> list[dict[str, object]]:
        rows = list(
            self.db.execute(
                ...
                .order_by(
                    NarrativeMemorySnapshot.snapshot_time.desc(),
                    NarrativeMemorySnapshot.heat_score.desc(),
                )
        ...
        return [
            {
                ...
                "historical_trend": (
                    "decaying" if row.decay_score > row.strength_score else "strengthening"
                ),
            }
            for row in rows
        ]
    ...
    def candle_api_payload(self, candle_id: int) -> dict[str, object]:
        ...
        return {
            ...
            "chart_markers": [
                item.model_dump()
                for item in self.news_markers(limit=1000)
                if item.candle_id == candle_id
                or item.event_id in {event.get("event_id") for event in candle.candidate_events}
            ],
    ...
    def timeline_for_candle(self, candle_id: int, limit: int = 50) -> list[dict[str, object]]:
        rows = list(
            self.db.execute(
                ...
                .where(
                    IntelligenceTimelineEvent.related_candle_id == candle_id,
                    IntelligenceTimelineEvent.is_deleted.is_(False),
                )
                .order_by(
    ...
    def timeline_for_event(self, event_id: int, limit: int = 50) -> list[dict[str, object]]:
        rows = list(
            self.db.execute(
                select(IntelligenceTimelineEvent)
                .where(
                    IntelligenceTimelineEvent.related_event_id == event_id,
                    IntelligenceTimelineEvent.is_deleted.is_(False),
                )
                .order_by(
                    IntelligenceTimelineEvent.event_time.asc(), IntelligenceTimelineEvent.id.asc()
    ...
    def _normalize_filters(self, raw_filters: list[str] | str) -> list[str]:
        ...
        aliases = {
            ...
            "etf": "institutional",
            "high confidence only": "high_confidence",
            "operator reviewed only": "operator_reviewed",
        }
        for part in parts:
    ...
    def _apply_filter(self, query: Any, filter_name: str) -> Any:
        ...
        if filter_name == "operator_reviewed":
            return query.where(
                IntelligenceTimelineEvent.event_type.in_(["operator_review", "publication"])
            )
        if filter_name == "operator_actions":
            return query.where(
                IntelligenceTimelineEvent.event_type.in_(
                    ["operator_review", "publication", "operator_action"]
                )
            )
    ...
    def _apply_window(self, query: Any, window: str) -> Any:
        ...
        if window in windows:
            return query.where(
                IntelligenceTimelineEvent.event_time >= datetime.utcnow() - windows[window]
            )
        return query
    ...
    def _candidate_event(
        self, candidate: CandleAttributionCandidate, event: NewsEvent | None
    ) -> dict[str, object]:
        return {
    ...
    def _candidate_article(
        self, candidate: CandleAttributionCandidate, article: NewsArticle | None
    ) -> dict[str, object]:
        return {
    ...
    def _similarity_preview(self, row: dict[str, object]) -> dict[str, object]:
        ...
        pattern = (
            row.get("matched_pattern") or row.get("pattern") or row.get("pattern_code") or "unknown"
        )
        return {
            "similar_event": similar_event.get("title") or row.get("title") or "Historical event",
            "date": similar_event.get("first_seen_at")
            or row.get("date")
