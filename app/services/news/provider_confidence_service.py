...
class ProviderConfidenceService:
    ...
    def apply_health_result(
        self, db: Session, source: NewsSource, result: HealthResult
    ) -> SourceHealthRecord:
        ...
        rec = SourceHealthRecord(
            ...
            provider_confidence_before=before,
            provider_confidence_after=source.provider_confidence,
            failure_count_snapshot=source.failure_count,
            success_count_snapshot=source.success_count,
            backoff_until=source.backoff_until,
            error_message=result.error_message,
            metadata_json={"is_degraded": source.is_degraded},
        )
        db.add(rec)
    ...
    def build_health_snapshot(
        self, db: Session, source: NewsSource, window: str
    ) -> SourceHealthSnapshot:
        records = list(
            db.execute(
                select(SourceHealthRecord)
                .where(SourceHealthRecord.source_id == source.id)
                .order_by(SourceHealthRecord.id.desc())
                .limit(200)
            ).scalars()
        )
        total = len(records)
        ...
        snap = SourceHealthSnapshot(
            ...
            failure_rate=((total - success) / total if total else 0.0),
            ...
            consecutive_successes=source.consecutive_successes,
            last_success_at=source.last_success_at,
            last_failure_at=source.last_failure_at,
            degraded_state=source.is_degraded,
            health_band=source.health_band,
        )
        db.add(snap)
        db.commit()
        db.refresh(snap)
