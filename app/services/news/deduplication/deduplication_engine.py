...
from app.services.news.deduplication.constants import ALGORITHM_VERSION, DeduplicationStatus, NORMALIZATION_VERSION
...
class DeduplicationEngine:
    def process_article(self, db: Session, article: NewsArticle) -> NewsArticle:
        ...
        for cand in existing:
            sim = calculate_similarity({"title": article.title, "canonical_url_hash": article.canonical_url_hash, "content_hash": article.content_hash}, {"title": cand.title, "canonical_url_hash": cand.canonical_url_hash, "content_hash": cand.content_hash})
        ...
        article.deduplication_metadata_json = {"algorithm_version": ALGORITHM_VERSION, "normalization_version": NORMALIZATION_VERSION, "matched_article_ids": [best[0].id] if best else []}
        db.commit()
