...
def find_cluster_candidates(db: Session, article: NewsArticle) -> list[NewsArticle]:
    ...
    stmt = select(NewsArticle).where(
        NewsArticle.id != article.id, NewsArticle.published_at >= recent
    )
...
def cluster_article(db: Session, article_id: int) -> int | None:
    ...
    if best is None or best_score < 0.75:
        cl = NewsArticleCluster(
            cluster_key=f"cl-{article.id}",
...
def recluster_recent_articles(db: Session) -> int:
    ids = [
        x
        for x, in db.execute(
            select(NewsArticle.id).where(NewsArticle.cluster_id.is_(None)).limit(200)
        ).all()
    ]
    for aid in ids:
