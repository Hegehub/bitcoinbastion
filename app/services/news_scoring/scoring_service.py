...
class NewsScoringService:
    ...
    def score_article(self, article: NewsArticle) -> NewsArticleScore:
        ...
        conf = self.calculate_confidence(
            article.provider_confidence, btc, [inst, macro, reg, sec], article.published_at
        )
        breakdown = self.build_factor_breakdown(
            btc, impact, inst, macro, reg, sec, sov, sent.factors
        )
    ...
    def calculate_confidence(
        self, source_cred: float, keyword_density: float, cats: list[float], published_at: datetime
    ) -> float:
    ...
    def build_factor_breakdown(
        ...
        return {
            "btc_relevance": [f"score={btc:.2f}"],
            "market_impact": [f"score={impact:.2f}"],
            "categories": {
                "institutional": inst,
                "macro": macro,
                "regulatory": reg,
    ...
    def build_limitations(self, confidence: float) -> dict[str, object]:
        items = [
            "Rule-based scoring may miss nuance.",
            "No external ML model used.",
            "Correlation is not proof of causation.",
            "Provider corroboration not yet available.",
        ]
        if confidence < 0.45:
    ...
    def derive_risk_band(
        self, security: float, macro: float, regulatory: float, confidence: float
    ) -> str:
        if confidence < 0.3:
            return "UNKNOWN"
        if security >= 0.9:
