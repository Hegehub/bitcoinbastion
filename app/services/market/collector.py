...
class BTCPriceCollector:
    def collect(self, db: Session) -> BTCMarketContext:
        ...
        round_id = hashlib.sha256(
            str(
                db.execute(
                    select(BTCPricePoint.id).order_by(BTCPricePoint.id.desc()).limit(1)
                ).scalar()
                or 0
            ).encode()
        ).hexdigest()[:16]
            ...
            try:
                ...
                db.add(
                    BTCPricePoint(
                        ...
                        provider_latency_ms=p.latency_ms,
                        aggregation_round_id=round_id,
                        is_median_selected=False,
                        metadata_json={
                            "raw_payload_hash": hashlib.sha256(
                                json.dumps(p.raw_payload, sort_keys=True).encode()
                            ).hexdigest()
                        },
                    )
                )
        ...
        if context.providers:
            ...
            rows = list(
                db.execute(
                    select(BTCPricePoint).where(BTCPricePoint.aggregation_round_id == round_id)
                ).scalars()
            )
            for row in rows:
    ...
    def providers_health(self, db: Session) -> list[MarketProviderHealth]:
        return list(
            db.execute(
                select(MarketProviderHealth).order_by(MarketProviderHealth.updated_at.desc())
            ).scalars()
        )
