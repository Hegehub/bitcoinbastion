    ...
    def aggregate(self, points: list[ProviderPrice]) -> BTCMarketContext:
        ...
        if not valid:
            return BTCMarketContext(
                0.0, 0, 0.0, 0.0, [], True, "no_provider_available", datetime.now(UTC)
            )
        ...
        reason = (
            "only_one_provider_available"
            if count <= 1
            else "provider_spread_too_large" if spread > 2.5 else ""
        )
        return BTCMarketContext(
            float(median(prices)),
            count,
            round(max(0.0, min(1.0, conf)), 4),
            round(spread, 4),
            [
                {"provider": p.provider, "price": p.price_usd, "latency_ms": p.latency_ms}
                for p in used
            ],
            degraded,
            reason,
