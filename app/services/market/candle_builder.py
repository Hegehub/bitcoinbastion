...
class MarketCandleBuilderService:
    def build(self, db: Session, timeframe: str, at: datetime) -> BTCCandle | None:
        ...
        if not points:
            db.add(
                CandleBuildRun(
                    timeframe=timeframe, window_start=start, window_end=end, build_status="no_data"
                )
            )
            db.commit()
            return None
        ...
        if existing is None:
            candle = MarketTimeSeriesRepository(db).insert_candle(
                ...
                close=cl,
                volume=None,
                price_source_mode="median_multi_provider",
                provider_count=len(providers),
                provider_confidence=conf,
                provider_disagreement_score=disagreement,
                aggregation_method="hierarchical_v1",
                ...
                evidence_packet_id="",
                provider_snapshot_json={"providers": list(providers)},
                integrity_status="degraded" if degraded else "valid",
                integrity_notes="provider_disagreement" if disagreement > 0.02 else "",
            ...
            for prv in providers:
                db.add(
                    CandleProviderSnapshot(
                        candle_id=candle.id,
                        provider_name=prv,
                        provider_price_open=op,
                        provider_price_high=hi,
                        provider_price_low=lo,
                        provider_price_close=cl,
                        provider_confidence=conf,
                    )
                )
        ...
        db.add(
            CandleBuildRun(
                ...
                degraded_reason=(
                    ...
                    if len(providers) <= 1
                    else "provider_disagreement" if disagreement > 0.02 else ""
                ),
            )
        )
        db.commit()
