...
ACTIVE_ACCESS_STATUSES = {
    ...
    ENTITLEMENT_STATUS_GRACE,
    ENTITLEMENT_STATUS_CANCELLED,
}
...
class SubscriptionEntitlementService:
    ...
    def issue_entitlement(
        ...
        *,
        ...
        self._validate_issue_inputs(
            pass_lookup_hash, certificate_fingerprint, valid_from, valid_until
        )
        if (
            self.get_current_entitlement(
                pass_lookup_hash=pass_lookup_hash, include_restricted=False
            )
            is not None
        ):
            raise EntitlementIntegrityError(
                "Active entitlement already exists and must be replaced explicitly"
            )
        ...
        *,
        ...
        for entitlement in self.db.execute(statement).scalars():
            ...
            if (
                ...
                and entitlement.status == ENTITLEMENT_STATUS_CANCELLED
                and _naive_utc(entitlement.valid_until) <= now
            ):
                continue
            return entitlement
        return None
    ...
    def freeze_entitlement(
        self, entitlement: SubscriptionEntitlement, reason: str = "frozen"
    ) -> SubscriptionEntitlement:
    ...
    def calculate_metric_entitlements(
    ...
    def validate_entitlement_for_metric(
        ...
        if metric_group not in metrics.groups:
            ...
            return AccessDecision(
                ...
                if required_plan
                else PolicyDecision.METRIC_NOT_ALLOWED,
        ...
        if metric_name is not None:
            ...
            if definition is None or get_metric_group_for_metric(metric_name) != metric_group:
                return AccessDecision(
                    PolicyDecision.METRIC_NOT_ALLOWED,
                    "unknown_metric",
                    requested_metric_group=metric_group,
                )
            estimated_cost = (
                estimated_cost if estimated_cost is not None else get_metric_cost(metric_name)
            )
        if (
            estimated_cost is not None
            and metrics.daily_metric_credits is not None
            and estimated_cost > metrics.daily_metric_credits
        ):
            return AccessDecision(
                PolicyDecision.QUOTA_EXCEEDED,
                "metric_credit_quota_exceeded",
                requested_metric_group=metric_group,
            )
        if (
            interval is not None
            and not self.validate_entitlement_for_interval(entitlement, interval).decision
            == PolicyDecision.ALLOW
        ):
            return self.validate_entitlement_for_interval(entitlement, interval)
        if (
            history_days is not None
            and not self.validate_entitlement_for_history_range(entitlement, history_days).decision
            == PolicyDecision.ALLOW
        ):
    ...
    def validate_entitlement_for_scope(
        self, entitlement: SubscriptionEntitlement, requested_scope: str
    ) -> AccessDecision:
        status_decision = self._status_decision(entitlement)
        if status_decision is not None:
            return status_decision
        if requested_scope in FORBIDDEN_SCOPES or requested_scope not in ACCESS_SCOPES:
            return AccessDecision(
                PolicyDecision.DENY, "scope_denied", requested_scope=requested_scope
            )
        if requested_scope not in self.calculate_effective_scopes(entitlement):
            return AccessDecision(
                PolicyDecision.INSUFFICIENT_SCOPE,
                "insufficient_scope",
                requested_scope=requested_scope,
            )
        return AccessDecision(
            PolicyDecision.ALLOW, "scope_allowed", requested_scope=requested_scope
        )

    def validate_entitlement_for_interval(
        self, entitlement: SubscriptionEntitlement, interval: str
    ) -> AccessDecision:
        status_decision = self._status_decision(entitlement)
        if status_decision is not None:
            return status_decision
    ...
    def validate_entitlement_for_history_range(
        self, entitlement: SubscriptionEntitlement, history_days: int
    ) -> AccessDecision:
        status_decision = self._status_decision(entitlement)
        if status_decision is not None:
            return status_decision
        if history_days < 0 or not validate_history_range_allowed(
            normalize_plan_code(entitlement.plan_code), history_days
        ):
    ...
    def verify_entitlement_signature(
        self, entitlement: SubscriptionEntitlement, public_key: str | None = None
    ) -> bool:
        ...
        if verification_key is None:
            raise EntitlementSignatureError(
                "Issuer public key is required for entitlement verification"
            )
        legacy_valid = verify_subscription_entitlement_signature(
            payload, verification_key, signature
        ).valid
        if not legacy_valid:
            return False
        if entitlement.issuer_envelope_json:
            return verify_serialized_issuer_envelope(
                payload,
                entitlement.issuer_envelope_json,
                public_key=verification_key,
                expected_key_id=entitlement.issuer_key_id,
            )
        return True
    ...
    def to_public_response(self, entitlement: SubscriptionEntitlement) -> dict[str, Any]:
        return {
            ...
            "metric_groups": sorted(
                (entitlement.metric_entitlements_json or {}).get("groups") or []
            ),
            "scopes": sorted(entitlement.scopes_json or []),
            "limits": dict(entitlement.limits_json or {}),
            "crypto_epoch": entitlement.crypto_epoch,
            "issuer_key_id": entitlement.issuer_key_id,
            "signature_requirement_policy": entitlement.signature_requirement_policy,
            "crypto_assurance": entitlement.crypto_assurance or "classical",
            "requires_reissue": entitlement.requires_reissue,
    ...
    def _create_entitlement(
        ...
        *,
        ...
        signature = sign_subscription_entitlement(
            payload, self.issuer_private_key, self.issuer_key_id, self.crypto_epoch
        )
        signature_json = {
            "alg": signature.alg,
        ...
        object_type = (
            ...
            if metadata.get("principal_type") == "lightning_wallet_principal"
            else BastionIssuedObjectType.WALLET_SUBSCRIPTION_ENTITLEMENT
        )
        payload_fingerprint = sha256_prefixed(canonical_json(payload))
        issuer_envelope = build_classical_issuer_envelope(
            payload,
            object_type=object_type,
            object_id_hash=pass_lookup_hash,
            object_fingerprint=payload_fingerprint,
            issuer_key_id=self.issuer_key_id,
    ...
    def _status_decision(self, entitlement: SubscriptionEntitlement) -> AccessDecision | None:
        ...
        if (
            entitlement.status == ENTITLEMENT_STATUS_EXPIRED
            or _naive_utc(entitlement.valid_until) <= now
        ):
            return AccessDecision(
        ...
        if self.issuer_public_key is not None and not self.verify_entitlement_signature(
            entitlement
        ):
            return AccessDecision(
                PolicyDecision.INVALID_SIGNATURE,
    ...
    def _refresh_signature(self, entitlement: SubscriptionEntitlement) -> None:
        ...
        object_type = (
            BastionIssuedObjectType.LIGHTNING_SUBSCRIPTION_ENTITLEMENT
            if (entitlement.metadata_json or {}).get("principal_type")
        ...
        envelope = build_classical_issuer_envelope(
            payload,
            object_type=object_type,
            object_id_hash=entitlement.pass_lookup_hash,
            object_fingerprint=sha256_prefixed(canonical_json(payload)),
            issuer_key_id=self.issuer_key_id,
            issuer_private_key=self.issuer_private_key,
            crypto_epoch=self.crypto_epoch,
        ...
        entitlement.issuer_envelope_json = envelope_json
        entitlement.issuer_envelope_hash = sha256_prefixed(canonical_json(envelope_json))
        entitlement.signature_requirement_policy = envelope.required_signature_policy.value
        entitlement.crypto_assurance = envelope.assurance_level.value
        entitlement.requires_reissue = False
