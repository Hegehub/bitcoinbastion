    ...
    def calculate(self, source_names: list[str]) -> EvidenceIndependenceResult:
        ...
        if count == 0:
            return EvidenceIndependenceResult(
                ...
                reason_codes=["EVIDENCE_INDEPENDENCE_LOW"],
            )
        ...
        if dom_share > 0.8:
            score = min(score, 0.5)
        code = (
            "EVIDENCE_INDEPENDENCE_LOW"
            if score < 0.35
            else "EVIDENCE_INDEPENDENCE_MEDIUM" if score < 0.75 else "EVIDENCE_INDEPENDENCE_HIGH"
        )
        return EvidenceIndependenceResult(
            score=score,
            source_count=count,
            independent_source_count=uniq,
