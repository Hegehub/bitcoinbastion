...
from app.db.models.bastion_trace import TraceBatch, TraceBatchItem, TraceBusinessEventModel, TraceBusinessPolicyProfileModel, TraceReport
...
from app.schemas.bastion_trace import BatchTraceItemResult, BatchTraceRequest, BusinessPolicyAction, BusinessTraceEventType, PaymentContextRiskRequest

from app.schemas.bastion_trace import BastionTraceRegisterAdvisoryRequest, BastionTraceTreasuryCheckRequest, EvidenceAccessRequest
...
from app.services.bastion_trace.trace_metrics import TRACE_BATCH, TRACE_CONF, TRACE_LITE, TRACE_PRIVACY, TRACE_REPORTS, TRACE_REQUESTS, TRACE_RUNTIME, TRACE_SCORE_BAND
...
class TraceService:
    ...
    def analyze_address(self, address: str) -> TraceReportSchema:
        ...
        disagreement = ProviderDisagreementService().detect_disagreement(["unknown"], [scoring.band.value])
        ...
        guidance = [self._guidance(scoring.band.value), "Do not provide seed phrases, private keys or wallet files."]
        breakdown = TraceScoreBreakdown(
            base_score=0.0,
            final_score=scoring.final_score,
            band=scoring.band,
            confidence=max(0.0, scoring.confidence - disagreement.confidence_impact + independence.score * 0.1),
            ...
            reason_codes=sorted(set(scoring.reason_codes + BASELINE_REASONS + disagreement.reason_codes + independence.reason_codes + passport.reason_codes)),
        ...
        create_event("TRACE_REPORT_CREATED", "INFO", "analyze_address", "success", "trace report created")
        TRACE_RUNTIME.labels(event_type="TRACE_REPORT_CREATED", severity="INFO", status="success").inc()
    ...
    def get_counterparty_lens(self, report_id: int) -> dict[str, object] | None:
        ...
        lens = build_counterparty_lens(report.address, report.trace_band, str(privacy.get("privacy_band", "UNKNOWN")), str(origin.get("origin_category", "unknown")), str(disag.get("severity", "NONE")), float(indep))
    ...
    def evaluate_payment_context(self, payload: PaymentContextRiskRequest) -> dict[str, object]:
        ...
        lens = build_counterparty_lens(payload.address, report.trace_band.value, "UNKNOWN", "unknown", "NONE", 0.0).model_dump(mode="json")
        ctx = evaluate_payment_context(payload.address, report.trace_band.value, report.confidence, "NONE", payload.amount_sats, payload.direction, payload.payment_purpose, payload.business_context == "treasury", payload.urgency == "urgent", lens)
    ...
    def build_lite_report(self, address: str) -> dict[str, object]:
        ...
        lite = LiteTraceReportService().from_trace_report(trace, str(privacy.get("privacy_band", "UNKNOWN")))
        TRACE_LITE.labels(status="success").inc()
        create_event("TRACE_LITE_CHECK_CREATED", "INFO", "lite_check", "success", "lite check created")
        TRACE_RUNTIME.labels(event_type="TRACE_LITE_CHECK_CREATED", severity="INFO", status="success").inc()
    ...
    def ensure_default_policy_profiles(self) -> None:
        ...
        for profile in default_policy_profiles():
            if profile.id in existing:
                continue
            self.repo.save_policy_profile(TraceBusinessPolicyProfileModel(id=profile.id, name=profile.name, description=profile.description, context_type=profile.context_type.value, low_action=profile.low_action.value, medium_action=profile.medium_action.value, high_action=profile.high_action.value, critical_action=profile.critical_action.value, unknown_action=profile.unknown_action.value, manual_review_threshold=profile.manual_review_threshold, high_value_threshold_sats=profile.high_value_threshold_sats, require_review_on_provider_disagreement=profile.require_review_on_provider_disagreement, require_review_on_low_confidence=profile.require_review_on_low_confidence, require_review_on_privacy_high=profile.require_review_on_privacy_high, limitations_json=json.dumps(profile.limitations)))
    ...
    def list_policy_profiles(self) -> list[dict[str, object]]:
        ...
        for p in self.repo.list_policy_profiles():
            out.append({"id": p.id, "name": p.name, "description": p.description, "context_type": p.context_type, "low_action": p.low_action, "medium_action": p.medium_action, "high_action": p.high_action, "critical_action": p.critical_action, "unknown_action": p.unknown_action, "manual_review_threshold": p.manual_review_threshold, "high_value_threshold_sats": p.high_value_threshold_sats, "require_review_on_provider_disagreement": p.require_review_on_provider_disagreement, "require_review_on_low_confidence": p.require_review_on_low_confidence, "require_review_on_privacy_high": p.require_review_on_privacy_high, "limitations": json.loads(p.limitations_json)})
        return out
    ...
    def screen_batch(self, payload: BatchTraceRequest) -> dict[str, object]:
        ...
        batch = self.repo.create_batch(TraceBatch(batch_label=payload.batch_label or "", business_context=payload.business_context.value, policy_profile_id=payload.policy_profile_id, total_addresses=len(payload.addresses), limitations_json=json.dumps(["baseline_batch_screening"])) )
        ...
        for addr in payload.addresses:
            try:
                ...
                manual = policy in {BusinessPolicyAction.HOLD_FOR_REVIEW, BusinessPolicyAction.REJECT_BY_POLICY, BusinessPolicyAction.INSUFFICIENT_INFORMATION}
                self.repo.add_batch_item(TraceBatchItem(batch_id=batch.id, address=report.address, report_id=report.id, status="processed", trace_band=report.trace_band.value, trace_score=report.trace_score, confidence=report.confidence, policy_action=policy.value, manual_review_recommended=manual))
                items.append(BatchTraceItemResult(address=report.address, status="processed", report_id=report.id, trace_band=report.trace_band.value, trace_score=report.trace_score, confidence=report.confidence, policy_action=policy, manual_review_recommended=manual))
            except ValueError as exc:
                reason = "sensitive_wallet_material_not_accepted" if "Sensitive wallet material" in str(exc) else "invalid_public_bitcoin_address"
                safe_address = "redacted"
                self.repo.add_batch_item(TraceBatchItem(batch_id=batch.id, address=safe_address, status="rejected", rejection_reason=reason, manual_review_recommended=False))
                items.append(BatchTraceItemResult(address=safe_address, status="rejected", rejection_reason=reason, limitations=["BATCH_ADDRESS_REJECTED_INVALID"]))
        TRACE_BATCH.labels(status="attempt").inc()
        result = make_batch_result(batch.id, payload.batch_label, payload.business_context, items, ["max_addresses_per_batch=1000"])
    ...
    def evaluate_evidence_access_enterprise(self, payload: EvidenceAccessRequest) -> dict[str, object]:
        decision = decide_evidence_access(payload.requester_role.value)
        return {"evidence_ref": payload.evidence_ref, "requester_role": payload.requester_role.value, "purpose": payload.purpose, "decision": decision.value}

    def create_enterprise_proof_packet(self, report_id: int) -> dict[str, object]:
        return cast(dict[str, object], build_enterprise_proof_packet({"base_proof_packet": {"report_id": report_id}, "legal_hold_status": "UNKNOWN", "rbac_context": {"production_enforced": False}, "sso_context": default_sso_config().model_dump(mode="json"), "limitations": ["enterprise_baseline_placeholder"]}).model_dump(mode="json"))
    ...
    def get_policy_facts(self, report_id: int) -> dict[str, object] | None:
        ...
        return {
            ...
            "privacy_band": "UNKNOWN",
            "provider_disagreement_severity": "NONE",
            "safe_to_send_advisory": "MANUAL_REVIEW_RECOMMENDED" if report.trace_band in {"HIGH","CRITICAL","MEDIUM"} else "PROCEED_WITH_CAUTION",
    ...
    def treasury_destination_check(self, payload: BastionTraceTreasuryCheckRequest) -> dict[str, object]:
        ...
        result: dict[str, object] = {
            ...
            "safe_to_send_advisory": "MANUAL_REVIEW_RECOMMENDED" if report.trace_band.value in {"HIGH","CRITICAL","MEDIUM"} else "PROCEED_WITH_CAUTION",
    ...
    def create_trace_alert_placeholder(self, alert_type: str, severity: str, message: str) -> dict[str, object]:
    ...
    def get_trace_status(self) -> dict[str, object]:
        ...
        last = last_report.created_at.isoformat() if last_report and last_report.created_at else None
        return make_status(reports_count=reports, lite_checks_count=0, batches_count=0, source_count=len(self.repo.list_sources()), last_report_at=last)
    ...
    def _validate_public_address(self, address: str) -> str:
        ...
        if any(t in lowered for t in ["xprv", "xpriv", ".dat", "seed phrase", "mnemonic"]) or len(value.split()) >= 12:
            raise ValueError("Sensitive wallet material is not accepted. Use only a public Bitcoin address.")
        if re.match(r"^(0x)[0-9a-fA-F]{40}$", value):
            raise ValueError("Invalid Bitcoin address")
        if re.match(r"^[KL5][1-9A-HJ-NP-Za-km-z]{50,51}$", value):
            raise ValueError("Sensitive wallet material is not accepted. Use only a public Bitcoin address.")
        if re.match(r"^(1|3)[1-9A-HJ-NP-Za-km-z]{25,62}$", value) or re.match(r"^bc1[ac-hj-np-z02-9]{11,71}$", lowered):
            return value
        raise ValueError("Invalid Bitcoin address")
