...
NEWS_FETCH_FAILURES_TOTAL = Counter(
    "news_fetch_failures_total", "Total news fetch failures", ["source"]
)
NEWS_FETCH_DURATION_SECONDS = Histogram(
    "news_fetch_duration_seconds", "News fetch duration seconds", ["source"]
)
NEWS_ARTICLES_INGESTED_TOTAL = Counter(
    "news_articles_ingested_total", "News articles ingested", ["source"]
)
NEWS_DUPLICATE_CANDIDATES_TOTAL = Counter(
    "news_duplicate_candidates_total", "Duplicate candidates", ["source"]
)
