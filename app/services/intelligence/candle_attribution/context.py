...
class CandleContextBuilder:
    def summarize(self, candle: BTCCandle, candidates: list[CandidateScore]) -> dict[str, object]:
        return {
            ...
            "positive_event_count": sum(1 for item in candidates if str(item.event.event_sentiment).upper() == "POSITIVE"),
            "negative_event_count": sum(1 for item in candidates if str(item.event.event_sentiment).upper() == "NEGATIVE"),
        }
