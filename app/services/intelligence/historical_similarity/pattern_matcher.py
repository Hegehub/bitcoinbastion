...
class PatternMatcher:
    def identify(self, event: NewsEvent) -> list[PatternMatch]:
        ...
        matches = [PatternMatch(primary.value, self._score(primary), f"Primary pattern inferred from event title/category: {primary.value}.")]
        text = f"{event.canonical_title} {event.canonical_summary}".lower()
        secondary = {
            MarketPattern.SECURITY_INCIDENT: ["hack", "exploit", "malware"],
            MarketPattern.BANKING_STRESS if hasattr(MarketPattern, "BANKING_STRESS") else MarketPattern.MACRO_RISK_OFF: ["bank", "liquidity stress"],
        ...
        for pattern, keywords in secondary.items():
            if pattern != primary and any(keyword in text for keyword in keywords):
                matches.append(PatternMatch(pattern.value, 0.58, f"Secondary pattern matched keyword(s): {', '.join(keywords)}."))
