...
def webhook_signature(*, secret: str, timestamp: int, raw_body: str) -> str:
...
def verify_signature(
    *,
    ...
    """Verify a Bastion webhook signature with timestamp replay tolerance.

    ``delivery_id`` is accepted for caller compatibility; receivers should still
    persist delivery ids separately for replay detection.
    """
    del delivery_id
    ...
    if observed < current - tolerance_seconds or observed > current + tolerance_seconds:
        return False
    expected = webhook_signature(secret=secret, timestamp=observed, raw_body=raw_body)
