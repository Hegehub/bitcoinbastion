...
from app.services.market_data.metrics import SUCCESSFUL_COLLECTIONS, PROVIDER_FAILURES_TOTAL, PROVIDER_CONFIDENCE
...
class MarketDataService:
    def collect_btc_price(self, db: Session) -> dict[str, object]:
        ...
        for provider in get_providers():
            try:
                ...
                points.append(p)
                db.add(BTCPricePoint(provider=p.provider, pair=p.pair, price_usd=p.price_usd, observed_at=p.observed_at, latency_ms=p.latency_ms, provider_confidence=p.provider_confidence, raw_payload_hash=p.raw_payload_hash, metadata_json=p.metadata_json))
                ...
                row = record_provider_result(db, provider.get_provider_name(), False, None, None, str(exc))
                PROVIDER_FAILURES_TOTAL.labels(provider=provider.get_provider_name()).inc()
                PROVIDER_CONFIDENCE.labels(provider=provider.get_provider_name()).set(row.provider_confidence)
        db.commit()
    ...
    def latest_points(self, db: Session, limit: int = 200) -> list[BTCPricePoint]:
        return list(db.execute(select(BTCPricePoint).order_by(BTCPricePoint.observed_at.desc()).limit(limit)).scalars())
