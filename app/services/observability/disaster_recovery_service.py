...
class DisasterRecoveryService:
    ...
    def verify_backup(
        ...
        *,
        ...
        row = BackupValidationRecord(
            ...
            or ([] if success else ["backup validation did not verify integrity"]),
        )
        db.add(row)
        db.commit()
        db.refresh(row)
    ...
    def verify_restore(
        ...
        *,
        recovery_id: str,
        validation_type: str = "full_restore",
        replay_types: list[str] | None = None,
        integrity_verified: bool = True,
        deterministic_rebuild_verified: bool = True,
        limitations: list[str] | None = None,
    ) -> RecoveryValidationOut:
        now = datetime.utcnow()
        ...
        row = RecoveryValidationRecord(
            ...
            limitations=limitations
            or ([f"missing replay validations: {', '.join(missing)}"] if missing else []),
        )
        db.add(row)
        db.commit()
        db.refresh(row)
    ...
    def validate_integrity_replay(self, db: Session, *, recovery_id: str) -> RecoveryValidationOut:
        return self.verify_restore(
            db,
            recovery_id=recovery_id,
            validation_type="integrity_replay",
    ...
    def _backup_out(self, row: BackupValidationRecord) -> BackupValidationOut:
        return BackupValidationOut(
            backup_id=row.backup_id,
            started_at=row.started_at,
            finished_at=row.finished_at,
            success=row.success,
            objects_checked=row.objects_checked,
    ...
    def _recovery_out(self, row: RecoveryValidationRecord) -> RecoveryValidationOut:
        return RecoveryValidationOut(
            recovery_id=row.recovery_id,
            validation_type=row.validation_type,
            started_at=row.started_at,
            finished_at=row.finished_at,
            success=row.success,
            deterministic_rebuild_verified=row.deterministic_rebuild_verified,
            integrity_verified=row.integrity_verified,
            replay_types=list(row.replay_types or []),
            limitations=list(row.limitations or []),
        )
