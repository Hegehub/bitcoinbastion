...
BASTION_HTTP_REQUESTS_TOTAL = Counter(
    "bastion_http_requests_total", "Total Bastion HTTP requests", ["endpoint", "status"]
)
BASTION_HTTP_REQUEST_DURATION_SECONDS = Histogram(
    "bastion_http_request_duration_seconds", "Bastion HTTP request duration", ["endpoint", "status"]
)
SIGNAL_LATENCY = Histogram(
    "signal_generation_latency_seconds", "Signal generation latency", ["source"]
)
...
def validate_bounded_labels(labels: dict[str, str]) -> dict[str, str]:
    return {
        key: bounded_label(key, value) for key, value in labels.items() if key in BOUNDED_LABELS
    }
...
RSS_FETCH_FAILURES_TOTAL = Counter(
    "rss_fetch_failures_total", "RSS fetch failures", ["provider", "reason_code"]
)
BTC_PRICE_COLLECTION_TOTAL = Counter(
    "btc_price_collection_total", "BTC price collection attempts", ["provider", "status"]
)
BTC_PRICE_COLLECTION_FAILURES_TOTAL = Counter(
    "btc_price_collection_failures_total",
    "BTC price collection failures",
    ["provider", "reason_code"],
)
NEWS_EVENTS_CREATED_TOTAL = Counter("news_events_created_total", "News events created", ["status"])
NEWS_IMPACTS_CREATED_TOTAL = Counter(
    "news_impacts_created_total", "News impacts created", ["status"]
)
CANDLE_ATTRIBUTIONS_CREATED_TOTAL = Counter(
    "candle_attributions_created_total", "Candle attributions created", ["status"]
)
SIGNALS_CREATED_TOTAL = Counter(
    "signals_created_total", "Signals created", ["signal_type", "status"]
)
SIGNALS_PUBLISHED_TOTAL = Counter(
    "signals_published_total", "Signals published", ["signal_type", "status"]
)
SIGNALS_REJECTED_TOTAL = Counter(
    "signals_rejected_total", "Signals rejected", ["signal_type", "reason_code"]
)
SIGNALS_PENDING_REVIEW_TOTAL = Gauge(
    "signals_pending_review_total", "Signals pending operator review", ["signal_type"]
)
TELEGRAM_PUBLICATIONS_TOTAL = Counter(
    "telegram_publications_total", "Telegram publication attempts", ["status"]
)
TELEGRAM_PUBLICATION_FAILURES_TOTAL = Counter(
    "telegram_publication_failures_total", "Telegram publication failures", ["reason_code"]
)
PROVIDER_DEGRADED_TOTAL = Counter(
    "provider_degraded_total", "Provider degraded events", ["provider", "reason_code"]
)
BACKGROUND_JOB_FAILURES_TOTAL = Counter(
    "background_job_failures_total", "Background job failures", ["job_name", "reason_code"]
)
BACKGROUND_JOB_DURATION_SECONDS = Histogram(
    "background_job_duration_seconds", "Background job duration seconds", ["job_name", "status"]
)
OPERATOR_REVIEWS_TOTAL = Counter(
    "operator_reviews_total", "Operator reviews", ["status", "reason_code"]
)
BASTION_NEWS_FETCH_TOTAL = Counter(
    "bastion_news_fetch_total", "Bastion news fetch attempts", ["provider", "status"]
)
BASTION_NEWS_FETCH_FAILURES_TOTAL = Counter(
    "bastion_news_fetch_failures_total", "Bastion news fetch failures", ["provider", "status"]
)
BASTION_PROVIDER_HEALTH_SCORE = Gauge(
    "bastion_provider_health_score", "Bastion provider health score", ["provider"]
)
BASTION_PRICE_PROVIDER_HEALTH_SCORE = Gauge(
    "bastion_price_provider_health_score", "Bastion price provider health score", ["provider"]
)
BASTION_NEWS_EVENTS_TOTAL = Counter("bastion_news_events_total", "Bastion news events", ["status"])
BASTION_CANDLE_ATTRIBUTIONS_TOTAL = Counter(
    "bastion_candle_attributions_total", "Bastion candle attributions", ["status"]
)
BASTION_SIGNALS_GENERATED_TOTAL = Counter(
    "bastion_signals_generated_total", "Bastion generated signals", ["signal_type", "status"]
)
BASTION_SIGNALS_PUBLISHED_TOTAL = Counter(
    "bastion_signals_published_total", "Bastion published signals", ["signal_type", "status"]
)
BASTION_EVIDENCE_PACKETS_TOTAL = Counter(
    "bastion_evidence_packets_total", "Bastion evidence packets", ["status"]
)
BASTION_REPLAY_REQUESTS_TOTAL = Counter(
    "bastion_replay_requests_total", "Bastion replay requests", ["status"]
)
BASTION_OPERATOR_REVIEWS_TOTAL = Counter(
    "bastion_operator_reviews_total", "Bastion operator reviews", ["status"]
)
BASTION_BACKGROUND_JOBS_TOTAL = Counter(
    "bastion_background_jobs_total", "Bastion background jobs", ["job_name", "status"]
)
BASTION_BACKGROUND_JOB_FAILURES_TOTAL = Counter(
    "bastion_background_job_failures_total",
    "Bastion background job failures",
    ["job_name", "status"],
)
BASTION_PROVIDER_DEGRADED_TOTAL = Counter(
    "bastion_provider_degraded_total", "Bastion provider degraded events", ["provider", "status"]
)

NEWS_ARTICLES_PROCESSED_TOTAL = Counter(
    "news_articles_processed_total", "News articles processed", ["provider_type", "status"]
)
MARKET_PRICE_POINTS_TOTAL = Counter(
    "market_price_points_total", "Market price points collected", ["provider_type", "status"]
)
BTC_CANDLES_GENERATED_TOTAL = Counter(
    "btc_candles_generated_total", "BTC candles generated", ["timeframe", "status"]
)
PRICE_IMPACTS_GENERATED_TOTAL = Counter(
    "price_impacts_generated_total", "Price impacts generated", ["status"]
)
CANDLE_ATTRIBUTIONS_GENERATED_TOTAL = Counter(
    "candle_attributions_generated_total", "Candle attributions generated", ["timeframe", "status"]
)
HISTORICAL_SIMILARITY_QUERIES_TOTAL = Counter(
    "historical_similarity_queries_total", "Historical similarity queries", ["status"]
)
SIGNALS_GENERATED_TOTAL = Counter(
    "signals_generated_total", "Signals generated", ["signal_type", "status"]
)
SIGNALS_BLOCKED_TOTAL = Counter(
    "signals_blocked_total", "Signals blocked", ["signal_type", "status"]
)
PROVIDER_HEALTH_FAILURES_TOTAL = Counter(
    "provider_health_failures_total", "Provider health failures", ["provider_type", "status"]
)
TIMELINE_BUILD_FAILURES_TOTAL = Counter(
    "timeline_build_failures_total", "Timeline build failures", ["status"]
)
CRONJOB_FAILURES_TOTAL = Counter(
    "cronjob_failures_total", "CronJob failures", ["job_name", "status"]
)
DR_RECOVERY_RUNS_TOTAL = Counter("dr_recovery_runs_total", "Disaster recovery runs", ["status"])
BACKUP_VALIDATION_RUNS_TOTAL = Counter(
    "backup_validation_runs_total", "Backup validation runs", ["status"]
)

OBSERVABILITY_METRIC_NAMES = [
    "rss_fetch_total",
    "rss_fetch_failures_total",
    "btc_price_collection_total",
    "btc_price_collection_failures_total",
    "news_events_created_total",
    "news_impacts_created_total",
    "candle_attributions_created_total",
    "signals_created_total",
    "signals_published_total",
    "signals_rejected_total",
    "signals_pending_review_total",
    "evidence_packets_generated_total",
    "evidence_replay_requests_total",
    "telegram_publications_total",
    "telegram_publication_failures_total",
    "provider_degraded_total",
    ...
    "bastion_news_fetch_failures_total",
    "bastion_provider_health_score",
    "bastion_price_provider_health_score",
    "bastion_news_events_total",
    "bastion_candle_attributions_total",
    "bastion_signals_generated_total",
    "bastion_signals_published_total",
    "bastion_evidence_packets_total",
    "bastion_replay_requests_total",
    "bastion_operator_reviews_total",
    "bastion_background_jobs_total",
    "bastion_background_job_failures_total",
    "bastion_provider_degraded_total",
    "news_articles_processed_total",
    "market_price_points_total",
    "btc_candles_generated_total",
    "price_impacts_generated_total",
    "candle_attributions_generated_total",
    "historical_similarity_queries_total",
    "signals_generated_total",
    "signals_blocked_total",
    "provider_health_failures_total",
    "timeline_build_failures_total",
...
OBS_PROVIDER_LATENCY_MS = Gauge(
    "obs_provider_latency_ms", "Provider probe latency ms", ["provider_type", "provider_name"]
)
OBS_PROVIDER_FALLBACK_ACTIVE = Gauge(
    "obs_provider_fallback_active",
    "Provider fallback active (1/0)",
    ["provider_type", "provider_name"],
)
OBS_PROVIDER_CONFIDENCE = Gauge(
    "obs_provider_confidence", "Provider confidence", ["provider_type", "provider_name"]
)
OBS_PROVIDER_LAST_SUCCESS_AGE_SECONDS = Gauge(
    "obs_provider_last_success_age_seconds",
    "Provider age since last successful evidence",
    ["provider_type", "provider_name"],
)
OBS_PROVIDER_FAILURE_COUNT = Counter(
    "obs_provider_failure_total",
    "Provider failure count",
    ["provider_type", "provider_name", "error_type"],
)


def _safe_provider_labels(provider_type: str, provider_name: str) -> tuple[str, str]:
    safe_type = (
        provider_type if provider_type in {"bitcoin", "rss", "delivery", "unknown"} else "unknown"
    )
...
class MetricsMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        BASTION_HTTP_REQUESTS_TOTAL.labels(
            endpoint=endpoint,
            status=bounded_label("status", "success" if response.status_code < 500 else "failure"),
        ).inc()
        BASTION_HTTP_REQUEST_DURATION_SECONDS.labels(
            endpoint=endpoint,
            status=bounded_label("status", "success" if response.status_code < 500 else "failure"),
        ).observe(duration)
        return response
...
def set_observability_runtime_metrics(
    *,
    ...
    OBS_PROVIDER_SHARE.labels(provider=(provider_name or "unknown")).set(
        max(0.0, min(1.0, float(provider_share)))
    )
...
def set_provider_health_detail_metrics(
    *,
    ...
    safe_type, safe_name = _safe_provider_labels(provider_type, provider_name)
    OBS_PROVIDER_LATENCY_MS.labels(provider_type=safe_type, provider_name=safe_name).set(
        float(max(0, latency_ms))
    )
    OBS_PROVIDER_FALLBACK_ACTIVE.labels(provider_type=safe_type, provider_name=safe_name).set(
        1.0 if is_fallback else 0.0
    )
    OBS_PROVIDER_CONFIDENCE.labels(provider_type=safe_type, provider_name=safe_name).set(
        max(0.0, min(1.0, confidence))
    )
    OBS_PROVIDER_LAST_SUCCESS_AGE_SECONDS.labels(
        provider_type=safe_type, provider_name=safe_name
    ).set(float(max(0, last_success_age_seconds)))


def increment_provider_failure_metric(
    *, provider_type: str, provider_name: str, error_type: str | None
) -> None:
    safe_type, safe_name = _safe_provider_labels(provider_type, provider_name)
    OBS_PROVIDER_FAILURE_COUNT.labels(
        provider_type=safe_type, provider_name=safe_name, error_type=(error_type or "none")[:64]
    ).inc()
