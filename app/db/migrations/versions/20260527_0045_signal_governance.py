...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("requires_operator_review", sa.Boolean(), nullable=False, server_default=sa.true()),
        sa.Column("published_at", sa.DateTime(), nullable=True),
        sa.Column("created_at", sa.DateTime(), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(), nullable=False, server_default=sa.func.now()),
    )
    for col in ["signal_type", "source_entity_type", "source_entity_id", "article_id", "event_id", "candle_id", "impact_id", "attribution_id", "confidence_score", "status", "policy_decision", "requires_operator_review", "published_at", "created_at"]:
        op.create_index(f"ix_intelligence_signal_candidates_{col}", "intelligence_signal_candidates", [col])

    op.create_table(
        "intelligence_operator_reviews",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("signal_candidate_id", sa.Integer(), sa.ForeignKey("intelligence_signal_candidates.id"), nullable=False),
    ...
    for col in ["signal_candidate_id", "review_status", "reviewer_id", "false_positive_marker", "created_at"]:
        op.create_index(f"ix_intelligence_operator_reviews_{col}", "intelligence_operator_reviews", [col])
    ...
    op.create_table(
        ...
        sa.Column("require_review_for_security_shock", sa.Boolean(), nullable=False, server_default=sa.true()),
        sa.Column("require_review_for_regulatory_shock", sa.Boolean(), nullable=False, server_default=sa.true()),
        sa.Column("require_review_for_low_confidence", sa.Boolean(), nullable=False, server_default=sa.true()),
        sa.Column("require_review_for_provider_degraded", sa.Boolean(), nullable=False, server_default=sa.true()),
        sa.Column("require_review_for_false_signal", sa.Boolean(), nullable=False, server_default=sa.true()),
    ...
    op.create_index("ix_intelligence_publishing_policies_name", "intelligence_publishing_policies", ["name"], unique=True)
    op.create_index("ix_intelligence_publishing_policies_is_active", "intelligence_publishing_policies", ["is_active"])
    op.bulk_insert(
        sa.table(
            "intelligence_publishing_policies",
            sa.column("name"),
    ...
    for col in ["signal_candidate_id", "channel", "delivery_status", "created_at"]:
        op.create_index(f"ix_intelligence_signal_delivery_logs_{col}", "intelligence_signal_delivery_logs", [col])


def downgrade() -> None:
    for col in ["created_at", "delivery_status", "channel", "signal_candidate_id"]:
        op.drop_index(f"ix_intelligence_signal_delivery_logs_{col}", table_name="intelligence_signal_delivery_logs")
    op.drop_table("intelligence_signal_delivery_logs")
    op.drop_index("ix_intelligence_publishing_policies_is_active", table_name="intelligence_publishing_policies")
    op.drop_index("ix_intelligence_publishing_policies_name", table_name="intelligence_publishing_policies")
    op.drop_table("intelligence_publishing_policies")
    for col in ["created_at", "false_positive_marker", "reviewer_id", "review_status", "signal_candidate_id"]:
        op.drop_index(f"ix_intelligence_operator_reviews_{col}", table_name="intelligence_operator_reviews")
    op.drop_table("intelligence_operator_reviews")
    for col in ["created_at", "published_at", "requires_operator_review", "policy_decision", "status", "confidence_score", "attribution_id", "impact_id", "candle_id", "event_id", "article_id", "source_entity_id", "source_entity_type", "signal_type"]:
        op.drop_index(f"ix_intelligence_signal_candidates_{col}", table_name="intelligence_signal_candidates")
