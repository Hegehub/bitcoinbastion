...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "primary_narrative", sa.String(length=128), nullable=False, server_default="unknown"
        ),
        sa.Column(
    ...
    for column_name in ["event_type", "pattern_type", "event_id", "article_id", "created_at"]:
        op.create_index(
            f"ix_historical_event_profiles_{column_name}",
            "historical_event_profiles",
            [column_name],
        )
    ...
    for column_name in ["reference_event_id", "candidate_event_id", "similarity_score"]:
        op.create_index(
            f"ix_historical_similarity_results_{column_name}",
            "historical_similarity_results",
            [column_name],
        )
...
def downgrade() -> None:
    ...
    for column_name in ["created_at", "article_id", "event_id", "pattern_type", "event_type"]:
        op.drop_index(
            f"ix_historical_event_profiles_{column_name}", table_name="historical_event_profiles"
        )
