...
TAXONOMY = [
    ...
    ("institutional-adoption", "INSTITUTIONAL_ADOPTION", "Institutional Adoption", "Institutional allocation and adoption narratives."),
    ("treasury-adoption", "TREASURY_ADOPTION", "Treasury Adoption", "Treasury and reserve-asset adoption narratives."),
    ("self-custody", "SELF_CUSTODY", "Self Custody", "Self-custody and withdrawal narratives."),
    ("sovereignty", "SOVEREIGNTY", "Sovereignty", "Sovereignty and censorship-resistance narratives."),
    ("bitcoin-core", "BITCOIN_CORE", "Bitcoin Core", "Bitcoin Core software and protocol maintenance narratives."),
    ...
    ("interest-rates", "INTEREST_RATES", "Interest Rates", "Rate-cut, rate-hike, and yield narratives."),
    ...
    ("exchange-liquidity", "EXCHANGE_LIQUIDITY", "Exchange Liquidity", "Exchange liquidity and order-book narratives."),
    ("exchange-failure", "EXCHANGE_FAILURE", "Exchange Failure", "Exchange solvency, halt, and failure narratives."),
    ...
    ("wallet-security", "WALLET_SECURITY", "Wallet Security", "Wallet and key-management security narratives."),
    ("privacy", "PRIVACY", "Privacy", "Bitcoin privacy narratives."),
    ("nation-state-adoption", "NATION_STATE_ADOPTION", "Nation State Adoption", "Nation-state adoption and reserve narratives."),
    ("corporate-adoption", "CORPORATE_ADOPTION", "Corporate Adoption", "Corporate Bitcoin adoption narratives."),
    ("energy", "ENERGY", "Energy", "Energy, grid, and mining power narratives."),
    ("layer2", "LAYER2", "Layer2", "Bitcoin Layer 2 narratives."),
    ("stablecoins", "STABLECOINS", "Stablecoins", "Stablecoin and dollar-rail narratives around Bitcoin markets."),
    ("market-structure", "MARKET_STRUCTURE", "Market Structure", "Market structure, basis, volatility, and order-book narratives."),
    ("liquidation-cascade", "LIQUIDATION_CASCADE", "Liquidation Cascade", "Liquidation cascade and forced-deleveraging narratives."),
...
KEYWORDS = {
    ...
    "interest-rates": [("interest rates", 2.0), ("rate cut", 1.5), ("rate hike", 1.5), ("yields", 1.2)],
    ...
    "exchange-failure": [("exchange failure", 2.5), ("withdrawals halted", 2.0), ("insolvency", 1.8)],
    ...
    "nation-state-adoption": [("nation state", 2.2), ("sovereign", 1.4), ("strategic reserve", 1.8)],
    ...
    "liquidation-cascade": [("liquidation cascade", 2.7), ("liquidations", 2.0), ("forced deleveraging", 1.7)],
...
def upgrade() -> None:
    op.add_column("market_narratives", sa.Column("narrative_type", sa.String(length=64), nullable=False, server_default=""))
    op.add_column("market_narratives", sa.Column("display_name", sa.String(length=160), nullable=False, server_default=""))
    op.create_index("ix_market_narratives_narrative_type", "market_narratives", ["narrative_type"])

    op.add_column("narrative_snapshots", sa.Column("narrative_type", sa.String(length=64), nullable=False, server_default=""))
    op.add_column("narrative_snapshots", sa.Column("heat_score", sa.Float(), nullable=False, server_default="0"))
    op.add_column("narrative_snapshots", sa.Column("volume_score", sa.Float(), nullable=False, server_default="0"))
    op.add_column("narrative_snapshots", sa.Column("growth_score", sa.Float(), nullable=False, server_default="0"))
    op.create_index("ix_narrative_snapshots_narrative_type", "narrative_snapshots", ["narrative_type"])
    ...
    op.create_index("ix_narrative_observations_narrative_type", "narrative_observations", ["narrative_type"])
    op.create_index("ix_narrative_observations_article_id", "narrative_observations", ["article_id"])
    op.create_index("ix_narrative_observations_event_id", "narrative_observations", ["event_id"])
    op.create_index("ix_narrative_observations_observed_at", "narrative_observations", ["observed_at"])

    conn = op.get_bind()
    for slug, narrative_type, display_name, description in TAXONOMY:
        row = conn.execute(sa.text("select id from market_narratives where slug = :slug"), {"slug": slug}).first()
        if row is None:
            conn.execute(
                sa.text("insert into market_narratives (slug, narrative_type, name, display_name, description, category, is_active) values (:slug, :type, :name, :display, :description, :category, 1)"),
                {"slug": slug, "type": narrative_type, "name": display_name, "display": display_name, "description": description, "category": narrative_type.lower()},
            )
        else:
            conn.execute(
                sa.text("update market_narratives set narrative_type = :type, display_name = :display where slug = :slug"),
                {"slug": slug, "type": narrative_type, "display": display_name},
            )
    conn.execute(sa.text("update market_narratives set narrative_type = upper(replace(slug, '-', '_')) where narrative_type = ''"))
    conn.execute(sa.text("update market_narratives set display_name = name where display_name = ''"))
