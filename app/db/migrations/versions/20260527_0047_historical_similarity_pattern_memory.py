...
def upgrade() -> None:
    op.add_column(
        "market_patterns",
        sa.Column("pattern_code", sa.String(length=96), nullable=False, server_default=""),
    )
    op.add_column(
        "market_patterns",
        sa.Column(
    ...
    op.add_column(
        "market_patterns",
        sa.Column(
            "default_impact_window", sa.String(length=32), nullable=False, server_default="1h"
        ),
    )
    op.add_column(
        "market_patterns",
        sa.Column("risk_profile", sa.String(length=64), nullable=False, server_default="standard"),
    )
    op.execute("UPDATE market_patterns SET pattern_code = slug WHERE pattern_code = ''")
    op.execute(
        "UPDATE market_patterns SET default_sentiment = expected_sentiment WHERE default_sentiment = 'UNKNOWN'"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Integer(),
            sa.ForeignKey(
                "market_patterns.id", name="fk_pattern_occurrences_pattern_id_market_patterns"
            ),
            nullable=False,
        ),
        ...
        sa.Column(
            "event_id",
            sa.Integer(),
            sa.ForeignKey("news_events.id", name="fk_pattern_occurrences_event_id_news_events"),
            nullable=True,
        ),
    ...
    with op.batch_alter_table("historical_similarity_results") as batch_op:
        batch_op.add_column(
            sa.Column(
                "source_event_id",
                sa.Integer(),
                sa.ForeignKey("news_events.id", name="fk_hsr_source_event_id_news_events"),
                nullable=True,
            )
        )
        batch_op.add_column(
            sa.Column(
                "pattern_id",
                sa.Integer(),
                sa.ForeignKey("market_patterns.id", name="fk_hsr_pattern_id_market_patterns"),
                nullable=True,
            )
        )
        batch_op.add_column(
        ...
        batch_op.create_index(
            "ix_historical_similarity_results_source_event_id", ["source_event_id"]
        )
    ...
    op.create_table(
        ...
        sa.Column(
            "occurrence_id", sa.Integer(), sa.ForeignKey("pattern_occurrences.id"), nullable=True
        ),
    ...
    op.add_column(
        "market_narratives",
        sa.Column("related_patterns", json_type, nullable=False, server_default="[]"),
    )
...
def downgrade() -> None:
    ...
    for col in ["direction", "reaction_window", "event_id", "occurrence_id", "pattern_id"]:
        op.drop_index(
            f"ix_pattern_reaction_snapshots_{col}", table_name="pattern_reaction_snapshots"
        )
