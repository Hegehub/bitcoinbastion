...
PATTERNS = [
    ...
    (
        ...
        "SEC enforcement",
        "regulatory",
        "SEC enforcement, litigation, or settlement pressure.",
    ),
    (
        "REGULATORY_APPROVAL",
        "Regulatory approval",
    ...
    (
        ...
        "Lightning adoption",
        "lightning",
        "Lightning Network adoption or infrastructure pattern.",
    ),
    (
        "TREASURY_ADOPTION",
        "Treasury adoption",
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_historical_similarity_matches_event_id", "historical_similarity_matches", ["event_id"]
    )
    op.create_index(
        "ix_historical_similarity_matches_similar_event_id",
        "historical_similarity_matches",
        ["similar_event_id"],
    )
    op.create_index(
        "ix_historical_similarity_matches_similarity_score",
        "historical_similarity_matches",
        ["similarity_score"],
    )
    ...
    op.create_index(
        "ix_historical_reaction_profiles_event_id", "historical_reaction_profiles", ["event_id"]
    )
    ...
    op.bulk_insert(
        sa.table("historical_patterns", *[sa.column(key) for key in rows[0].keys()]), rows
    )


def downgrade() -> None:
    op.drop_index(
        "ix_historical_reaction_profiles_event_id", table_name="historical_reaction_profiles"
    )
    op.drop_table("historical_reaction_profiles")
    op.drop_index(
        "ix_historical_similarity_matches_similarity_score",
        table_name="historical_similarity_matches",
    )
    op.drop_index(
        "ix_historical_similarity_matches_similar_event_id",
        table_name="historical_similarity_matches",
    ...
    op.drop_index(
        "ix_historical_similarity_matches_event_id", table_name="historical_similarity_matches"
    )
