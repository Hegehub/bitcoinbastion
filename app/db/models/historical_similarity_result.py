...
class HistoricalSimilarityResult(Base):
    ...
    source_event_id: Mapped[int | None] = mapped_column(ForeignKey("news_events.id"), nullable=True, index=True)
    ...
    reference_candle_id: Mapped[int | None] = mapped_column(ForeignKey("btc_candles.id"), nullable=True, index=True)
    candidate_event_id: Mapped[int | None] = mapped_column(ForeignKey("news_events.id"), nullable=True, index=True)
    pattern_id: Mapped[int | None] = mapped_column(ForeignKey("market_patterns.id"), nullable=True, index=True)
    matched_event_id: Mapped[int | None] = mapped_column(ForeignKey("news_events.id"), nullable=True, index=True)
    matched_signal_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    matched_article_id: Mapped[int | None] = mapped_column(ForeignKey("news_articles.id"), nullable=True, index=True)
