...
class EventFingerprintRecord(Base):
    ...
    event_id: Mapped[int] = mapped_column(ForeignKey("news_events.id"), unique=True, index=True, nullable=False)
    ...
    volatility_profile: Mapped[dict[str, object]] = mapped_column(JSONB().with_variant(JSON(), "sqlite"), default=dict)
