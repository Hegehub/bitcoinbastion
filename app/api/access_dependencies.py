...
from app.services.access.plan_entitlements import build_entitlement_overlay, required_plan_for_metric_group
...
ACCESS_SESSION_MISSING = "access_session_missing"
...
ACCESS_SCOPE_MISSING = "access_scope_missing"
...
async def get_access_context(request: Request, db: Annotated[Session, Depends(get_db)]) -> AccessContext:
    ...
    signed_headers_present = all(
        ...
        for name in ("x-bastion-timestamp", "x-bastion-nonce", "x-bastion-body-hash", "x-bastion-signature")
    )
...
async def _verify_signed_request(request: Request, db: Session) -> AccessContext:
    ...
    try:
        ...
        verified = AccessRequestVerifier(server_pepper=server_pepper).verify(db, method=request.method, path=request.url.path, body=body, headers=request.headers)
        ...
        raise access_error(ACCESS_SIGNATURE_INVALID, "Access request signature is invalid.", 401) from exc


def _context_from_session(session_token: str, request: Request, db: Session) -> AccessContext:
    try:
        session_context = SESSION_CONTEXT_RESOLVER(session_token, db) if SESSION_CONTEXT_RESOLVER is not None else _default_session_context(session_token, db)
    except Exception as exc:
    ...
    return AccessContext(
        ...
        request_id=getattr(request.state, "request_id", None),
        origin=request.headers.get("origin"),
        policy_mode=session_context.policy_mode if hasattr(session_context, "policy_mode") else "proof_of_access",
        is_request_signature_verified=False,
        metadata={"requires_request_signing": getattr(session_context, "requires_request_signing", True)},
    )
...
def _default_session_context(session_token: str, db: Session) -> Any:
    ...
    return AccessSessionService(db, server_pepper=server_pepper).validate_session(session_token=session_token)
...
async def require_access_session(context: Annotated[AccessContext, Depends(get_access_context)]) -> AccessContext:
...
def require_scope(scope: str) -> Any:
    async def dependency(context: Annotated[AccessContext, Depends(require_access_session)]) -> AccessContext:
...
def require_plan(plan_code: PlanCode | str) -> Any:
    ...
    async def dependency(context: Annotated[AccessContext, Depends(require_access_session)]) -> AccessContext:
        if plan_rank(context.plan_code) < plan_rank(required):
            raise access_error(ACCESS_UPGRADE_REQUIRED, f"Access plan {required.value} is required.", 402)
...
def require_metric_entitlement(metric_group: str) -> Any:
    async def dependency(context: Annotated[AccessContext, Depends(require_access_session)]) -> AccessContext:
        ...
        if metric_group not in groups:
            ...
            code = ACCESS_UPGRADE_REQUIRED if required_plan and plan_rank(required_plan) > plan_rank(context.plan_code) else ACCESS_METRIC_NOT_ALLOWED
            raise access_error(code, "Metric entitlement is not available for this Access plan.", 402 if code == ACCESS_UPGRADE_REQUIRED else 403)
...
def require_policy_decision(
    ...
    *,
    ...
    policy_context = AccessPolicyContext(
        ...
        step_up_present=access_context.is_step_up_verified or access_context.is_request_signature_verified,
        human_intent_verified=access_context.is_step_up_verified,
        metadata={**access_context.metadata, **(metadata or {}), "action": action} if action else {**access_context.metadata, **(metadata or {})},
    )
...
def require_step_up_for_critical_action(action: str) -> Any:
    async def dependency(context: Annotated[AccessContext, Depends(require_access_session)]) -> AccessContext:
        if not context.is_request_signature_verified:
            raise access_error(ACCESS_SIGNATURE_REQUIRED, "Critical Access action requires signed Bastion request headers.", 403)
        if not context.is_step_up_verified:
            raise access_error(ACCESS_STEP_UP_REQUIRED, "Critical Access action requires step-up or Human Intent Signature.", 403)
...
def _policy_error(decision: str, reason_code: str, message: str) -> AppError:
    ...
    code, status_code = mapping.get(decision, (ACCESS_POLICY_DENIED if reason_code != "scope_not_allowed" else ACCESS_SCOPE_MISSING, 403))
