...
def btc_candles_latest_any(
    db: Session = Depends(db_session), timeframe: str = "1m"
) -> dict[str, object]:
...
def btc_candle_evidence(candle_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(CandleProviderSnapshot).where(CandleProviderSnapshot.candle_id == candle_id)
        ).scalars()
    )
    return {
        "data": [
            {
                "provider_name": r.provider_name,
                "open": r.provider_price_open,
                "high": r.provider_price_high,
                "low": r.provider_price_low,
                "close": r.provider_price_close,
                "provider_confidence": r.provider_confidence,
            }
            for r in rows
        ]
...
def market_health(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    try:
        ...
        return {
            "data": {
                ...
                "global_market_confidence": 0.0,
                "median_provider_latency_ms": 0.0,
                "stale_provider_count": 0,
                "generated_at": datetime.now(UTC),
                "limitations": ["market_provider_health_table_unavailable"],
            }
        }
    ...
    return {
        "data": {
            ...
            "global_market_confidence": round(conf, 4),
            "median_provider_latency_ms": float(median(lats)) if lats else 0.0,
            "stale_provider_count": 0,
            "generated_at": datetime.now(UTC),
            "limitations": [],
        }
...
def btc_price_history(db: Session = Depends(db_session), limit: int = 200) -> dict[str, object]:
    ...
    try:
        rows = list(
            db.execute(
                select(BTCPricePoint).order_by(BTCPricePoint.observed_at.desc()).limit(limit)
            ).scalars()
        )
        ...
        "data": [
            {
                ...
                "provider_kind": "MARKET_PROVIDER",
                "symbol": r.symbol,
                "pair": r.pair,
                "price_usd": r.price_usd,
                "observed_at": r.observed_at,
                "provider_confidence": r.provider_confidence,
                "provider_latency_ms": r.provider_latency_ms or r.latency_ms,
                "provider_status": r.metadata_json.get("provider_status", "healthy"),
                "limitations": [],
            }
            for r in rows
        ]
