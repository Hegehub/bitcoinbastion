...
def list_evidence_packets(limit: int = 50, db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    return {"data": [builder.packet_payload(row) for row in rows], "limitations": EVIDENCE_SAFETY_LIMITATIONS}


@router.get("/packets/{packet_id}")
def get_evidence_packet(packet_id: int, format: str = "json", db: Session = Depends(db_session)) -> dict[str, object]:
    packet = EvidenceRepository(db).get_packet(packet_id)
    if packet is None:
        return {"data": None, "limitations": EVIDENCE_SAFETY_LIMITATIONS + ["Evidence packet was not found."]}
    builder = EvidencePacketBuilder(db)
    if format == "markdown":
        return {"format": "markdown", "content": builder.export_packet(packet, fmt="markdown"), "limitations": EVIDENCE_SAFETY_LIMITATIONS}
    return {"data": builder.packet_payload(packet), "limitations": EVIDENCE_SAFETY_LIMITATIONS}


@router.get("/packets/{packet_id}/timeline")
def get_evidence_packet_timeline(packet_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    packet = EvidenceRepository(db).get_packet(packet_id)
    if packet is None:
        return {"data": [], "limitations": EVIDENCE_SAFETY_LIMITATIONS + ["Evidence packet was not found."]}
    return {"data": EvidencePacketBuilder(db).timeline_for(packet.source_entity_type, packet.source_entity_id), "limitations": EVIDENCE_SAFETY_LIMITATIONS}


@router.get("/packets/{packet_id}/relationships")
def get_evidence_packet_relationships(packet_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    packet = EvidenceRepository(db).get_packet(packet_id)
    if packet is None:
        return {"data": [], "limitations": EVIDENCE_SAFETY_LIMITATIONS + ["Evidence packet was not found."]}
    builder = EvidencePacketBuilder(db)
    rows = builder.relationship_chain(packet.source_entity_type, packet.source_entity_id)
    return {"data": [builder.relationship_payload(row) for row in rows], "limitations": EVIDENCE_SAFETY_LIMITATIONS}


@router.get("/replay/{entity_type}/{entity_id}")
def replay_evidence(entity_type: str, entity_id: int, format: str = "json", db: Session = Depends(db_session)) -> dict[str, object]:
    service = EvidenceReplayService(db)
    if format == "markdown":
        content = service.export_replay(entity_type, entity_id, fmt="markdown")
        db.commit()
        return {"format": "markdown", "content": content, "limitations": EVIDENCE_SAFETY_LIMITATIONS}
...
def replay_evidence_timeline(entity_type: str, entity_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    return {"data": EvidenceReplayService(db).replay_timeline(entity_type, entity_id), "limitations": EVIDENCE_SAFETY_LIMITATIONS}


@router.get("/replay/{entity_type}/{entity_id}/integrity")
def replay_evidence_integrity(entity_type: str, entity_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
