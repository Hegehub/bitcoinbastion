...
from app.services.market_intelligence.source_registry import (
    SourceCategory,
...
def get_source(source_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    return {
        "data": {
            ...
            "signal_quality_weight": source.signal_quality_weight,
            "sovereignty_weight": source.sovereignty_weight,
...
def sources_health(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    return {
        "data": [
            {
                "source_id": x.id,
                "provider_confidence": x.provider_confidence,
                "health_band": x.health_band,
                "degraded_state": x.is_degraded,
...
def source_snapshots(source_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            ...
            .where(SourceHealthSnapshot.source_id == source_id)
            .order_by(SourceHealthSnapshot.id.desc())
            .limit(100)
        ).scalars()
    )
    return {
...
def source_confidence_events(
    source_id: int, db: Session = Depends(db_session)
) -> dict[str, object]:
    from sqlalchemy import select
    ...
    rows = list(
        db.execute(
            select(ProviderConfidenceEvent)
            .where(ProviderConfidenceEvent.source_id == source_id)
            .order_by(ProviderConfidenceEvent.id.desc())
            .limit(100)
        ).scalars()
    )
    return {
...
def list_clusters(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(NewsArticleCluster).order_by(NewsArticleCluster.id.desc()).limit(100)
        ).scalars()
    )
    return {
        "data": [
...
def get_cluster(cluster_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    arts = list(
        db.execute(select(NewsArticle).where(NewsArticle.cluster_id == cluster_id)).scalars()
    )
    return {
        "data": {
            "cluster_id": cl.id,
            "canonical_article_id": cl.canonical_article_id,
            "article_count": cl.article_count,
            "cluster_confidence": cl.cluster_confidence,
            "articles": [
                {"id": a.id, "title": a.title, "is_canonical": a.is_canonical} for a in arts
            ],
        }
    }
    ...
    rows = list(
        db.execute(
            select(NewsArticle).where(
                or_(NewsArticle.duplicate_of_id == article_id, NewsArticle.id == a.duplicate_of_id)
            )
        ).scalars()
    )
    return {
        "data": [
            {
                "id": x.id,
                "title": x.title,
                "deduplication_status": x.deduplication_status,
...
def list_events(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(NewsEvent)
            .where(NewsEvent.is_active.is_(True))
            .order_by(NewsEvent.first_seen_at.desc())
            .limit(100)
        ).scalars()
    )
    return {
...
def get_event_articles(event_id: int, db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(NewsArticle, NewsEventArticle)
...
def high_impact_events(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(NewsEvent)
            .where(NewsEvent.is_high_impact.is_(True), NewsEvent.is_active.is_(True))
            .order_by(NewsEvent.first_seen_at.desc())
            .limit(100)
        ).scalars()
    )
...
def security_events(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(NewsEvent)
            .where(NewsEvent.is_security_related.is_(True), NewsEvent.is_active.is_(True))
            .order_by(NewsEvent.first_seen_at.desc())
            .limit(100)
        ).scalars()
    )
    return {
        "data": [
            {"id": e.id, "canonical_title": e.canonical_title, "event_type": e.event_type}
            for e in rows
        ]
    }
...
def regulatory_events(db: Session = Depends(db_session)) -> dict[str, object]:
    ...
    rows = list(
        db.execute(
            select(NewsEvent)
            .where(NewsEvent.is_regulatory_related.is_(True), NewsEvent.is_active.is_(True))
            .order_by(NewsEvent.first_seen_at.desc())
            .limit(100)
        ).scalars()
    )
