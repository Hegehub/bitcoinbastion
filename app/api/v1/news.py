...
def latest_news(
    ...
    items = [
        NewsArticleOut.model_validate(item) for item in repo.latest(limit=limit, offset=offset)
    ]
...
@router.post(
    "/sources/reputation/refresh", response_model=ResponseEnvelope[list[SourceReputationProfileOut]]
)
def refresh_source_reputation(
...
@router.get(
    "/sources/reputation", response_model=ResponseEnvelope[list[SourceReputationProfileOut]]
)
...
def get_article_score(
    article_id: int, db: Session = Depends(db_session)
    ...
    return ResponseEnvelope(
        data={
            ...
            "scores": latest.factor_breakdown_json,
            "explanation": latest.explanation_json,
            "limitations": latest.limitations_json,
            "provider_confidence": latest.provider_confidence,
        }
    )
...
def get_event_score(
    ...
    latest = (
        db.query(NewsScore)
        .filter(NewsScore.event_id == event_id)
        .order_by(NewsScore.id.desc())
        .first()
    )
    if latest is None:
...
def high_impact_news(
    limit: int = 20, db: Session = Depends(db_session)
) -> ResponseEnvelope[list[dict[str, object]]]:
    rows = (
        db.query(NewsScore)
        .filter(NewsScore.market_impact_score >= 0.75)
        .order_by(NewsScore.id.desc())
        .limit(limit)
        .all()
    )
        ...
        data=[
            {
                ...
                "event_id": r.event_id,
                "market_impact_score": r.market_impact_score,
...
def security_news(
    ...
    rows = (
        db.query(NewsScore)
        .filter(NewsScore.security_risk_score >= 0.6)
        .order_by(NewsScore.id.desc())
        .limit(limit)
        .all()
    )
...
def regulatory_news(
    ...
    rows = (
        db.query(NewsScore)
        .filter(NewsScore.regulatory_score >= 0.6)
        .order_by(NewsScore.id.desc())
        .limit(limit)
        .all()
    )
...
def by_sentiment(
    label: str, limit: int = 20, db: Session = Depends(db_session)
) -> ResponseEnvelope[list[dict[str, object]]]:
    rows = (
        db.query(NewsScore)
        .filter(NewsScore.sentiment_score >= 0)
        .order_by(NewsScore.id.desc())
        .limit(limit)
        .all()
    )
...
def get_article_narratives(
    article_id: int, db: Session = Depends(db_session)
) -> ResponseEnvelope[list[dict[str, object]]]:
    from app.db.models.news_narrative_tag import NewsNarrativeTag

    rows = (
        db.query(NewsNarrativeTag)
        .filter(NewsNarrativeTag.article_id == article_id)
        .order_by(NewsNarrativeTag.id.desc())
        .all()
    )
    return ResponseEnvelope(
        data=[
            {"tag": r.tag, "confidence": r.confidence, "evidence": r.evidence_keywords_json}
            for r in rows
        ]
    )
...
def high_relevance(
    ...
    rows = (
        db.query(NewsScore)
        .filter(NewsScore.btc_relevance_score >= 0.75)
        .order_by(NewsScore.id.desc())
        .limit(limit)
        .all()
    )
    return ResponseEnvelope(
        data=[
            {
                "article_id": r.article_id,
                "btc_relevance_score": r.btc_relevance_score,
...
def _impact_payload(row: object) -> dict[str, object]:
    ...
    return {
        ...
        "impact_confidence": getattr(
            impact, "impact_confidence_score", getattr(impact, "confidence_score", 0.0)
        ),
        "confidence_band": getattr(
            impact, "impact_band", getattr(impact, "confidence_band", "VERY_LOW")
        ),
        ...
        "limitations": getattr(
            impact,
            "limitations_json",
            {"limitations": [getattr(impact, "limitation", "correlation_not_causation")]},
        ),
        "explanation": getattr(
            impact, "explanation_json", {"summary": getattr(impact, "explanation_summary", "")}
        ),
    }


@router.get("/{article_id}/impact")
def get_article_impact(
    article_id: int, db: Session = Depends(db_session)
) -> ResponseEnvelope[dict[str, object]]:
...
def get_article_explanation(
    ...
    exp = (
        ...
        .filter(ScoreExplanation.score_id == row.id)
    ...
    if exp is None:
        return ResponseEnvelope(
            data={
                "summary": row.explanation_json.get("summary", ""),
    ...
    return ResponseEnvelope(
        data={
            "summary": exp.summary,
            "key_factors": exp.key_factors_json,
            "limitations": exp.limitations_json,
        }
    )
