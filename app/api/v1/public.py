...
@router.get("/landing", response_model=ResponseEnvelope[PublicLandingResponse], summary="Public landing payload", description="This endpoint is advisory-only.")
...
@router.get("/status", response_model=ResponseEnvelope[PublicStatusResponse], summary="Public platform status")
...
@router.get("/roadmap", response_model=ResponseEnvelope[PublicRoadmapResponse], summary="Public roadmap summary")
def public_roadmap() -> ResponseEnvelope[PublicRoadmapResponse]:
    return ResponseEnvelope(data=get_roadmap())


@router.get("/stats", response_model=ResponseEnvelope[PublicStatsResponse], summary="Public-safe stats")
...
@router.get("/features", response_model=ResponseEnvelope[list[PublicFeatureEntry]], summary="Feature catalog")
def public_features() -> ResponseEnvelope[list[PublicFeatureEntry]]:
    return ResponseEnvelope(data=list_features())


@router.get("/trace/{report_id}/summary", response_model=ResponseEnvelope[PublicTraceSummary], summary="Public Trace summary")
def public_trace_summary(report_id: int, db: Session = Depends(db_session)) -> ResponseEnvelope[PublicTraceSummary]:
