...
- Files changed: `bastion_ui/navigation.py`, layout header/footer/mobile/sidebar/command-palette components, navigation state modules, command palette state module, and navigation/command-palette tests.
- Public nav status: required public items are centralized in `PUBLIC_NAV_ITEMS`; current non-root routes are marked preview until implemented.
- Footer nav status: footer uses the centralized public route registry and includes advisory/no-custody safety copy.
- Mobile nav status: mobile navigation includes all public routes plus `/check`, `/console`, and Trace safety copy.
- Console nav status: required console modules are present and marked coming soon until implemented.
- Command palette status: required static actions are registered; dynamic Trace report and Proof Packet actions are marked `requires_input=True`.
