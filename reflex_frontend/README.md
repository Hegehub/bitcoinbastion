...
Prompt 9 adds the Evidence and Proof Packet UI layer for Reflex:

- `/evidence` explains the Evidence layer, Proof Packet boundaries, provider disagreement, degraded states, and no-custody safety rules.
- `/trace/[report_id]/proof-packet` renders a route-driven Proof Packet view that shows unavailable/degraded states when backend data is missing.
- Evidence components render source badges, confidence badges, evidence cards, evidence chains, limitations, provider disagreement, and degraded evidence banners.
- The UI does not fabricate proof hashes, legal verdicts, payment approvals, or backend evidence values.
- Evidence remains advisory-only and source-dependent; it is not legal verification, not Bitcoin consensus proof, and not financial advice.
