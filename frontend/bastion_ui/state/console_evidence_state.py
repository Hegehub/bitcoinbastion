...
EVIDENCE_LISTING_ENDPOINT_MISSING = "Backend evidence listing endpoint is not available yet."


class ConsoleEvidenceState(rx.State):
    loading: bool = False
    error: str | None = None
    data: dict[str, Any] = {}
    ...
    async def refresh(self) -> None:
        ...
        self.data = {
            "available": False,
            "reason": EVIDENCE_LISTING_ENDPOINT_MISSING,
            "future_endpoint": "/api/v1/evidence",
        }
        self.degraded = True
        self.loading = False
    ...
    async def submit_report_lookup(self) -> None:
        ...
        if not validation.ok:
            ...
            return
        self.loading = True
        result = await EvidenceApiClient().get_trace_evidence(validation.report_id)
