...
def public_hero(title: str, eyebrow: str, description: str) -> rx.Component:
    return cast(
        ...
        rx.vstack(
            ...
            rx.hstack(
                rx.link(button("Open Trace", "primary"), href=path_for("trace")),
                rx.link(button("Developer API", "secondary"), href=path_for("developers")),
                rx.link(button("Operations", "ghost"), href=path_for("operations")),
                spacing="3",
                wrap="wrap",
            ),
            align="start",
