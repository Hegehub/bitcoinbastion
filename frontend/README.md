...
The Reflex frontend is now the sole user interface for Bitcoin Bastion. As of 2026-06-29 the legacy Next.js codebase and related CI/compose resources have been removed. This repository directory (`frontend/`) contains a Python-first UI built with Reflex that interacts with the FastAPI backend.
...
As of 2026-06-29 the legacy Next.js implementation has been deleted from the
repository. Its CI workflow and parallel Compose configurations remain removed;
the canonical `frontend/` path is now owned by this Reflex application. To
restore the legacy implementation for testing, check out the relevant commit
from Git history or consult
[`OLD_FRONTEND_REMOVAL_REPORT.md`](../docs/archive/audits/2026/OLD_FRONTEND_REMOVAL_REPORT.md).
Reflex is the only repository-native standalone frontend.
