...
export function SiteFooter({ language = 'en' }: { language?: SiteLanguage }) {
  ...
  const footerColumns = [
    { title: t.footer.product, links: [[t.nav.platform, '/platform'], [t.nav.trace, '/trace'], [t.nav.status, '/status'], [t.nav.roadmap, '/roadmap']] },
    { title: t.nav.developers, links: [[t.nav.docs, '/docs'], [t.nav.developers, '/developers'], [t.nav.evidence, '/evidence']] },
    { title: t.footer.operators, links: [[t.nav.operations, '/operations'], [t.nav.security, '/security'], [t.cta.viewStatus, '/status']] },
            ...
            <section key={col.title}>
              ...
              <ul className='mt-3 space-y-2'>
                {col.links.map(([label, href]) => (
                  <li key={`${label}-${href}`}><Link href={href} className='text-sm text-bb-graphite hover:text-bb-orange'>{label}</Link></li>
                ))}
              </ul>
            </section>
          ))}
