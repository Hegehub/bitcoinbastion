...
export const TraceStatusBanner = () => <div className='border rounded p-3 text-sm'>Bastion Trace provides heuristic evidence-based analysis and does not provide legal verification or Bitcoin consensus proof.</div>
...
export const TraceLimitationsCard = ({ s }: { s: PublicTraceSummary }) => <section><h2>Limitations</h2><ul>{(s.limitations || []).map((l) => <li key={l}>{l}</li>)}<li>Not legal verification</li><li>Not consensus proof</li></ul></section>
...
export const TraceProofPacketViewer = ({ id }: { id: string }) => <section><h1 className='text-2xl font-bold'>Proof Packet #{id}</h1><ul><li>Advisory-only</li><li>No custody</li><li>Not legal verification</li><li>Not Bitcoin consensus proof</li></ul><p>Proof packets are evidence bundles and not legal certificates.</p></section>
