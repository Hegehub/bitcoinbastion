...
def test_sensitive_inputs_rejected() -> None:
    assert (
        client.get(
            "/api/v1/trace/address/abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"
        ).status_code
        == 400
    )
    assert client.get("/api/v1/trace/address/xprv9s21ZrQH143K3abc").status_code == 400
    assert (
        client.get("/api/v1/trace/address/0x742d35Cc6634C0532925a3b844Bc454e4438f44e").status_code
        == 400
    )
...
def test_watchlist_and_sources_routes_work() -> None:
    post = client.post(
        "/api/v1/trace/watchlist",
        json={"address": "1BoatSLRHtKNngkdXEeobR76b53LETtpyT", "label": "test"},
    )
...
def test_lite_endpoint_works_and_rejects_sensitive() -> None:
    ...
    assert (
        client.get("/api/v1/trace/lite/0x742d35Cc6634C0532925a3b844Bc454e4438f44e").status_code
        == 400
    )
    assert (
        client.get(
            "/api/v1/trace/lite/abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"
        ).status_code
        == 400
    )
