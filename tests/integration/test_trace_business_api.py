...
def test_business_batch_endpoint() -> None:
    r = client.post('/api/v1/trace/business/batch', json={'addresses': ['bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kg3g4ty'], 'batch_label': 'b1'})
    assert r.status_code == 200
...
def test_business_batch_rejects_sensitive_input_without_secret_echo() -> None:
    ...
    r = client.post('/api/v1/trace/business/batch', json={'addresses': [secret_like], 'batch_label': 's1'})
    assert r.status_code == 200
