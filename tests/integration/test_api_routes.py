...
def test_new_api_groups_exist() -> None:
    ...
    try:
        ...
        assert (
            client.post(
                "/api/v1/citadel/simulations", json={"owner_id": 9, "scenario_code": "loss_signer"}
            ).status_code
            == 200
        )
        ...
        assert (
            client.post(
                "/api/v1/citadel/recalculate", json={"owner_type": "user", "owner_id": 9}
            ).status_code
            == 200
        )
