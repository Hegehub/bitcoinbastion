...
def test_task43_frontend_contract_dtos_and_marker_rendering() -> None:
    ...
    with SessionLocal() as session:
        _seed(session)
        dto = build_market_dto(
            __import__(
                "app.web.market_time_machine_service", fromlist=["MarketTimeMachineWebService"]
            )
            .MarketTimeMachineWebService(session)
            .dashboard(timeframe="1h"),
        ...
        assert dto["chart_data"]["supports"] == [
            "zoom",
            "pan",
            "hover",
            "candle_selection",
    ...
    try:
        ...
        assert (
            client.post(
                "/web/market-time-machine/marker-click?marker_type=regulatory_event"
            ).json()["status"]
            == "recorded"
        )
        assert (
            client.post("/web/market-time-machine/candle-click?timeframe=1h").json()["status"]
            == "recorded"
        )
        assert client.post("/web/market-time-machine/evidence-view").json()["status"] == "recorded"
        assert (
            client.post("/web/market-time-machine/replay-open?entity_type=event").json()["status"]
            == "recorded"
        )
    finally:
        app.dependency_overrides.clear()
