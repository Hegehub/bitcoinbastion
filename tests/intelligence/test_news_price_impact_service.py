...
def test_price_impact_article_calc() -> None:
    ...
    db.add(src)
    db.flush()
    a = NewsArticle(source_id=src.id, title="ETF approval inflow", normalized_title="etf approval inflow", raw_url="u", url="u", canonical_url="u", url_hash="u1", canonical_url_hash="c1", title_hash="t1", content_hash="h1", published_at=datetime.utcnow(), market_impact_score=0.8, btc_relevance_score=0.9)
    db.add(a)
    db.flush()
