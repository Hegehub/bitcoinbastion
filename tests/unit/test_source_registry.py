...
def test_baseline_confidence() -> None:
    ...
    assert svc.calculate_source_baseline_confidence(
        SourceTier.SOVEREIGN, SourceCategory.BITCOIN_CORE
    ) > svc.calculate_source_baseline_confidence(
        SourceTier.EXPERIMENTAL, SourceCategory.GENERAL_CRYPTO
    )
...
def test_activation_toggle(db: Session) -> None:
    ...
    assert (
        svc.get_source(db, created.id) is not None
        and svc.get_source(db, created.id).is_active is False
    )
