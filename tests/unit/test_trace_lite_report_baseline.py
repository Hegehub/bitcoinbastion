...
def test_lite_mapping_unknown() -> None:
    t = TraceReport(address="1BoatSLRHtKNngkdXEeobR76b53LETtpyT", trace_band=TraceBand.UNKNOWN, limitations=[])
...
def test_lite_contains_payloads_and_warnings() -> None:
    t = TraceReport(address="bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kygt080", trace_band=TraceBand.LOW, limitations=[])
    lite = LiteTraceReportService().from_trace_report(t)
