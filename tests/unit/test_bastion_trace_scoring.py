...
def test_empty_evidence_unknown() -> None:
    r = score_trace(
        TraceScoringInput(evidence_count=0, independent_source_count=0, baseline_mode=True)
    )
...
def test_baseline_cannot_high_or_critical() -> None:
    r = score_trace(
        TraceScoringInput(
            ...
            baseline_mode=True,
            factors={"known_high_risk_exposure": 1.0},
        )
    )
...
def test_threshold_mapping() -> None:
    low = score_trace(
        TraceScoringInput(
            ...
            baseline_mode=False,
            factors={"suspicious_pattern_signal": 0.5},
        )
    )
    med = score_trace(
        TraceScoringInput(
    ...
    high = score_trace(
        TraceScoringInput(
            ...
            independent_source_count=2,
            baseline_mode=False,
            factors={"known_high_risk_exposure": 1.0, "suspicious_pattern_signal": 1.0},
        )
    )
    crit = score_trace(
        TraceScoringInput(
            ...
            baseline_mode=False,
            factors={
                "known_high_risk_exposure": 1.0,
                "suspicious_pattern_signal": 1.0,
                "origin_uncertainty": 1.0,
                "privacy_leak_signal": 1.0,
            },
        )
    )
...
def test_dna_normalized() -> None:
    r = score_trace(
        TraceScoringInput(
            ...
            baseline_mode=False,
            factors={"privacy_leak_signal": 1.0},
        )
    )
