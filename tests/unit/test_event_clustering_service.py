...
def _source() -> NewsSource:
    return NewsSource(name="S", slug="s", kind="rss", base_url="https://x", rss_url="https://x/feed", category="bitcoin_media", tier="bitcoin_native", language="en", country_code="US")


def _article(source_id: int, title: str, ntitle: str, hours: int = 0) -> NewsArticle:
    now = datetime.utcnow() - timedelta(hours=hours)
    return NewsArticle(source_id=source_id, external_id="", title=title, normalized_title=ntitle, raw_url=f"https://x/{source_id}/{abs(hash(title))}", url=f"https://x/{source_id}/{abs(hash(title))}", canonical_url=f"https://x/{source_id}/{abs(hash(title))}", url_hash=f"{abs(hash(title))}1", canonical_url_hash=f"{abs(hash(title))}2", title_hash=f"{abs(hash(title))}3", content_hash=f"{abs(hash(title))}4", published_at=now)
