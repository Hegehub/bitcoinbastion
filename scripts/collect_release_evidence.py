...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Collect release evidence into artifacts JSON file."
    )
    parser.add_argument(
        "--output",
        default="",
    ...
    out = {
        ...
        "checks": [
            ...
            run("python scripts/run_postgres_migration_smoke.py"),
            run("python scripts/check_postgres_schema_parity.py"),
            run("python scripts/check_docs_truthfulness.py"),
        ],
        "health_readiness_result": run(
            "python - <<'PY'\nprint('health/readiness check placeholder: use deployed API probes')\nPY"
        ),
        "observability_snapshot": run(
            "python - <<'PY'\nprint('observability snapshot requires running API env')\nPY"
        ),
        "recovery_check": run(
            "python - <<'PY'\nprint('recovery-check requires running API env')\nPY"
        ),
        "metrics_scrape_result": run(
            "python - <<'PY'\nprint('metrics scrape requires running API env')\nPY"
        ),
        "known_limitations_acknowledgement": [
            "Some evidence fields require deployed environment/API credentials."
        ],
    }
